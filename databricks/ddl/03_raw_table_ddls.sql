...
CREATE TABLE IF NOT EXISTS raw.nyc_tlc.yellow_trips (
  ...
  RatecodeID INT COMMENT 'The final rate code in effect at the end of the trip. 1=Standard rate; 2=JFK; 3=Newark; 4=Nassau or Westchester; 5=Negotiated fare; 6=Group ride',
  ...
  fare_amount DECIMAL(10,2) COMMENT 'The time-and-distance fare calculated by the meter',
  extra DECIMAL(10,2) COMMENT 'Miscellaneous extras and surcharges. Currently, this only includes the 0.50USD and 1USD rush hour and overnight charges',
  mta_tax DECIMAL(10,2) COMMENT 'MTA tax that is automatically triggered based on the metered rate in use',
  tip_amount DECIMAL(10,2) COMMENT 'Tip amount. This field is automatically populated for credit card tips. Cash tips are not included',
  tolls_amount DECIMAL(10,2) COMMENT 'Total amount of all tolls paid in trip',
  improvement_surcharge DECIMAL(10,2) COMMENT 'Improvement surcharge assessed trips at the flag drop. The improvement surcharge began being levied in 2015',
  total_amount DECIMAL(10,2) COMMENT 'The total amount charged to passengers. Does not include cash tips',
